 ...
 * Task_NewGameBirchSpeech_StartBirchDiglettPlatformFade
 * Task_NewGameBirchSpeech_StartBirchDiglettPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchDiglett
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForDiglett
 * Task_NewGameBirchSpeechSub_WaitForDiglett
...
static void Task_NewGameBirchSpeechSub_WaitForDiglett(u8);
static void Task_NewGameBirchSpeech_StartBirchDiglettPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchDiglett(u8);
...
#define tDiglettSpriteId data[9]
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tDiglettSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, 0x0000FFFF, SPECIES_DIGLETT);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForDiglett;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForDiglett(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tDiglettSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(1);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchDiglettPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchDiglettPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tDiglettSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tDiglettSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tDiglettSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchDiglett;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchDiglett(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tDiglettSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tDiglettSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tDiglettSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateDiglettSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_DIGLETT, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    diglettSpriteId = NewGameBirchSpeech_CreateDiglettSprite(100, 0x4B);
    gSprites[diglettSpriteId].callback = SpriteCB_Null;
    gSprites[diglettSpriteId].oam.priority = 0;
    gSprites[diglettSpriteId].invisible = TRUE;
    gTasks[taskId].tDiglettSpriteId = diglettSpriteId;
