...
static void CopyMonToSummaryStruct(struct Pokemon *mon)
{
    ...
    else
    {
        ...
        SetMonData(mon, MON_DATA_HP, &boxMon[sMonSummaryScreen->curMonIndex].hp);
    }
}
...
static void Task_HandleInput(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (JOY_NEW(R_BUTTON))
        {
            if (sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS)
            {
                if (sMonSummaryScreen->statPageIndex == 2)
                    sMonSummaryScreen->statPageIndex = 0;
                else
                    sMonSummaryScreen->statPageIndex++;
        ...
        else if (JOY_NEW(L_BUTTON))
        {
            if (sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS)
            {
                if (sMonSummaryScreen->statPageIndex == 0)
                    sMonSummaryScreen->statPageIndex = 2;
                else
                    sMonSummaryScreen->statPageIndex--;
                BufferIvOrEvStats(sMonSummaryScreen->statPageIndex);
...
static void BufferLeftColumnStats(void)
{
    ...
    switch (sMonSummaryScreen->statPageIndex)
    {
    case 0: // iv mode
        hp = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_HP_IV);
        atk = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_ATK_IV);
    ...
    case 1:
        ...
        BufferStat(attackString, 0, atk, 1, 7);
        BufferStat(defenseString, 0, def, 2, 7);
        DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sStatsLeftColumnLayoutIVEV);
        break;
    case 2:
    default:
        BufferStat(currentHPString, 0, hp, 0, 3);
        BufferStat(maxHPString, 0, hp2, 1, 3);
        BufferStat(attackString, natureMod[STAT_ATK - 1], atk, 2, 7);
        BufferStat(defenseString, natureMod[STAT_DEF - 1], def, 3, 7);
        DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sStatsLeftColumnLayout);
        break;
    }
